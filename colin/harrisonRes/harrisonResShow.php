...
for ($i=0;$i<$resRayCnt; $i++)
{
	...
	if ($gDay > 0 )
	{
		...
		if (is_array($resCheckRay) === false)
			{
			$resCheckRayEmpty = makeDayRay($resRay[$i]);	
			$resCheckRay = $resCheckRayEmpty;
			}
	}
	
	if (($H0 > 0 || $H1 > 0) && $H7 === false && $propVar == "Harrison")
	{
		//echo("<br>Harrison");
		checkAvail($resRay[$i]);
	}
	...
	if (($TU0 > 0 || $TU1 > 0) && $TU99 === false && $propVar == "Tucker")
	{
		//echo("<br>Tucker House");
		checkAvail($resRay[$i]);
	}
	
	if (($TL0 > 0 || $TL1 > 0) && $TU99 === false && $propVar == "Tucker")
	{
